...
public class labs_student implements Initializable {
    ...
    public void cards() throws SQLException, IOException {
        ...
        Connection con = DriverManager.getConnection(oracleUrl, "N_LABS", "120120");
        con.setAutoCommit(false);
        String sql="select sec_num from REGST where STU_NUM="+id;
        ...
        sections.getItems().clear();
        while (rs.next()) {
            String sql2="SELECT distinct SEC_NUM,INSTRUCTOR.FIRST_NAME,INSTRUCTOR.LAST_NAME,LAB.NAME,SECTION.LAB_NUM FROM SECTION,INSTRUCTOR,LAB WHERE INSTRUCTOR.F_ID=SECTION.INS_NUM AND SECTION.sec_num ='"+rs.getString(1)+"' and SECTION.LAB_NUM=LAB.LAB_NUM";
